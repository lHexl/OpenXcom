 ...
 */
HostileFactionAI::HostileFactionAI(SavedBattleGame *save, BattleUnit *unit, Node *node) :
 ...
 */
HostileFactionAI::~HostileFactionAI()
{
 ...
 */
void HostileFactionAI::setTargetFaction(UnitFaction f)
{
	_targetFaction = f;
}
 ...
 */
void HostileFactionAI::load(const YAML::YamlNodeReader& reader)
{
 ...
 * @return YAML node.
 */
void HostileFactionAI::save(YAML::YamlNodeWriter writer) const
{
	writer.setAsMap();
	writer.setFlowStyle();
 ...
 */
void HostileFactionAI::dont_think(BattleAction *action)
{
	_melee = false;
 ...
 * @return Was it used?
 */
bool HostileFactionAI::medikit_think(BattleMediKitType healOrStim)
{
 ...
 * @param action (possible) AI action to execute after thinking is done.
 */
void HostileFactionAI::think(BattleAction *action)
{
 ...
 */
void HostileFactionAI::setWasHitBy(BattleUnit *attacker)
{
 ...
 */
void HostileFactionAI::setWeaponPickedUp()
{
 ...
 */
bool HostileFactionAI::getWasHitBy(int attacker) const
{
 ...
 */
void HostileFactionAI::setupPatrol()
{
 ...
 */
void HostileFactionAI::setupAmbush()
{
 ...
 */
void HostileFactionAI::setupAttack()
{
 ...
 */
void HostileFactionAI::setupEscape()
{
 ...
 */
int HostileFactionAI::countKnownTargets() const
{
	int knownEnemies = 0;
 ...
 * @return spotters.
 */
int HostileFactionAI::getSpottingUnits(const Position& pos) const
{
 ...
 */
int HostileFactionAI::selectNearestTarget()
{
	int tally = 0;
	_closestDist= 100;
	_aggroTarget = 0;
 ...
 * @return viable targets.
 */
int HostileFactionAI::selectNearestTargetLeeroy(bool canRun)
{
	int tally = 0;
	_closestDist = 100;
	_aggroTarget = 0;
 ...
 */
bool HostileFactionAI::selectClosestKnownEnemy()
{
	_aggroTarget = 0;
	int minDist = 255;
	for (auto* bu : *_save->getUnits())
 ...
 * @return if we found one.
 */
bool HostileFactionAI::selectRandomTarget()
{
	int farthest = -100;
	_aggroTarget = 0;
 ...
 */
bool HostileFactionAI::selectPointNearTarget(BattleUnit *target, int maxTUs)
{
 ...
 * @return True if a point was found.
 */
bool HostileFactionAI::selectPointNearTargetLeeroy(BattleUnit *target, bool canRun)
{
	int size = _unit->getArmor()->getSize();
 ...
 */
bool HostileFactionAI::selectSpottedUnitForSniper()
{
	_aggroTarget = 0;
 ...
 */
int HostileFactionAI::scoreFiringMode(BattleAction *action, BattleUnit *target, bool checkLOF)
{
 ...
 */
void HostileFactionAI::evaluateAIMode()
{
 ...
 */
bool HostileFactionAI::findFirePoint()
{
	if (!selectClosestKnownEnemy())
		return false;
 ...
 */
int HostileFactionAI::explosiveEfficacy(Position targetPos, BattleUnit *attackingUnit, int radius, int diff, bool grenade) const
{
 ...
 */
void HostileFactionAI::meleeAction()
{
	...
	if (_traceAI && _aggroTarget) { Log(LOG_INFO) << "HostileFactionAI::meleeAction:" << " [target]: " << (_aggroTarget->getId()) << " at: "  << _attackAction.target; }
 ...
 */
void HostileFactionAI::meleeActionLeeroy(bool canRun)
{
	if (_aggroTarget != 0 && !_aggroTarget->isOut())
	{
		if (_save->getTileEngine()->validMeleeRange(_unit, _aggroTarget, _save->getTileEngine()->getDirectionTo(_unit->getPosition(), _aggroTarget->getPosition())))
 ...
 */
void HostileFactionAI::wayPointAction()
{
 ...
 *
 */
bool HostileFactionAI::sniperAction()
{
 ...
 */
void HostileFactionAI::projectileAction()
{
	_attackAction.target = _aggroTarget->getPosition();
...
void HostileFactionAI::extendedFireModeChoice(BattleActionCost& costAuto, BattleActionCost& costSnap, BattleActionCost& costAimed, BattleActionCost& costThrow, bool checkLOF)
{
 ...
 */
void HostileFactionAI::grenadeAction()
{
 ...
 */
bool HostileFactionAI::psiAction()
{
 ...
 */
void HostileFactionAI::meleeAttack()
{
 ...
 * @return
 */
AIAttackWeight HostileFactionAI::getTargetAttackWeight(BattleUnit* target) const
{
 ...
 */
bool HostileFactionAI::validTarget(BattleUnit *target, bool assessDanger, bool includeCivs) const
{
	// ignore units that:
 ...
 */
BattleActionType HostileFactionAI::getReserveMode()
{
	return _reserve;
}
 ...
 */
void HostileFactionAI::selectMeleeOrRanged()
{
 ...
 */
bool HostileFactionAI::getNodeOfBestEfficacy(BattleAction *action, int radius)
{
	int bestScore = 2;
...
BattleUnit* HostileFactionAI::getTarget()
{
	return _aggroTarget;
}

void HostileFactionAI::freePatrolTarget()
{
	if (_toNode)
	{
		_toNode->freeNode();
