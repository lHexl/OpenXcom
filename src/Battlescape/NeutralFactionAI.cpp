 ...
 */
NeutralFactionAI::NeutralFactionAI(SavedBattleGame *save, BattleUnit *unit, Node *node) :
 ...
 */
NeutralFactionAI::~NeutralFactionAI()
{
 ...
 */
void NeutralFactionAI::setTargetFaction(UnitFaction f)
{
	_targetFaction = f;
}
 ...
 */
void NeutralFactionAI::load(const YAML::YamlNodeReader& reader)
{
 ...
 * @return YAML node.
 */
void NeutralFactionAI::save(YAML::YamlNodeWriter writer) const
{
	writer.setAsMap();
	writer.setFlowStyle();
 ...
 */
void NeutralFactionAI::dont_think(BattleAction *action)
{
	_melee = false;
 ...
 * @return Was it used?
 */
bool NeutralFactionAI::medikit_think(BattleMediKitType healOrStim)
{
 ...
 * @param action (possible) AI action to execute after thinking is done.
 */
void NeutralFactionAI::think(BattleAction *action)
{
 ...
 */
void NeutralFactionAI::setWasHitBy(BattleUnit *attacker)
{
 ...
 */
void NeutralFactionAI::setWeaponPickedUp()
{
 ...
 */
bool NeutralFactionAI::getWasHitBy(int attacker) const
{
 ...
 */
void NeutralFactionAI::setupPatrol()
{
 ...
 */
void NeutralFactionAI::setupAmbush()
{
 ...
 */
void NeutralFactionAI::setupAttack()
{
 ...
 */
void NeutralFactionAI::setupEscape()
{
 ...
 */
int NeutralFactionAI::countKnownTargets() const
{
	int knownEnemies = 0;
 ...
 * @return spotters.
 */
int NeutralFactionAI::getSpottingUnits(const Position& pos) const
{
 ...
 */
int NeutralFactionAI::selectNearestTarget()
{
	int tally = 0;
	_closestDist= 100;
	_aggroTarget = 0;
 ...
 * @return viable targets.
 */
int NeutralFactionAI::selectNearestTargetLeeroy(bool canRun)
{
	int tally = 0;
	_closestDist = 100;
	_aggroTarget = 0;
 ...
 */
bool NeutralFactionAI::selectClosestKnownEnemy()
{
	_aggroTarget = 0;
	int minDist = 255;
	for (auto* bu : *_save->getUnits())
 ...
 * @return if we found one.
 */
bool NeutralFactionAI::selectRandomTarget()
{
	int farthest = -100;
	_aggroTarget = 0;
 ...
 */
bool NeutralFactionAI::selectPointNearTarget(BattleUnit *target, int maxTUs)
{
 ...
 * @return True if a point was found.
 */
bool NeutralFactionAI::selectPointNearTargetLeeroy(BattleUnit *target, bool canRun)
{
	int size = _unit->getArmor()->getSize();
 ...
 */
bool NeutralFactionAI::selectSpottedUnitForSniper()
{
	_aggroTarget = 0;
 ...
 */
int NeutralFactionAI::scoreFiringMode(BattleAction *action, BattleUnit *target, bool checkLOF)
{
 ...
 */
void NeutralFactionAI::evaluateAIMode()
{
 ...
 */
bool NeutralFactionAI::findFirePoint()
{
	if (!selectClosestKnownEnemy())
		return false;
 ...
 */
int NeutralFactionAI::explosiveEfficacy(Position targetPos, BattleUnit *attackingUnit, int radius, int diff, bool grenade) const
{
 ...
 */
void NeutralFactionAI::meleeAction()
{
	...
	if (_traceAI && _aggroTarget) { Log(LOG_INFO) << "NeutralFactionAI::meleeAction:" << " [target]: " << (_aggroTarget->getId()) << " at: "  << _attackAction.target; }
 ...
 */
void NeutralFactionAI::meleeActionLeeroy(bool canRun)
{
	if (_aggroTarget != 0 && !_aggroTarget->isOut())
	{
		if (_save->getTileEngine()->validMeleeRange(_unit, _aggroTarget, _save->getTileEngine()->getDirectionTo(_unit->getPosition(), _aggroTarget->getPosition())))
 ...
 */
void NeutralFactionAI::wayPointAction()
{
 ...
 *
 */
bool NeutralFactionAI::sniperAction()
{
 ...
 */
void NeutralFactionAI::projectileAction()
{
	_attackAction.target = _aggroTarget->getPosition();
...
void NeutralFactionAI::extendedFireModeChoice(BattleActionCost& costAuto, BattleActionCost& costSnap, BattleActionCost& costAimed, BattleActionCost& costThrow, bool checkLOF)
{
 ...
 */
void NeutralFactionAI::grenadeAction()
{
 ...
 */
bool NeutralFactionAI::psiAction()
{
 ...
 */
void NeutralFactionAI::meleeAttack()
{
 ...
 * @return
 */
AIAttackWeight NeutralFactionAI::getTargetAttackWeight(BattleUnit* target) const
{
 ...
 */
bool NeutralFactionAI::validTarget(BattleUnit *target, bool assessDanger, bool includeCivs) const
{
	// ignore units that:
 ...
 */
BattleActionType NeutralFactionAI::getReserveMode()
{
	return _reserve;
}
 ...
 */
void NeutralFactionAI::selectMeleeOrRanged()
{
 ...
 */
bool NeutralFactionAI::getNodeOfBestEfficacy(BattleAction *action, int radius)
{
	int bestScore = 2;
...
BattleUnit* NeutralFactionAI::getTarget()
{
	return _aggroTarget;
}

void NeutralFactionAI::freePatrolTarget()
{
	if (_toNode)
	{
		_toNode->freeNode();
