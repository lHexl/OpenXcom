...
constexpr int PLAYER_AI_GRENADE_TEAM_MIN_SCORE_DEFAULT = 180; // Базовый minScore team-spotted explosive.
constexpr int PLAYER_AI_GRENADE_TEAM_MIN_SCORE_CLUSTER = 140; // MinScore team-spotted explosive по кластеру врагов.
constexpr int PLAYER_AI_GRENADE_TEAM_MIN_SCORE_HIGH = 150; // MinScore team-spotted explosive по очень ценной одиночной цели.
constexpr int PLAYER_AI_GRENADE_TEAM_MIN_SCORE_MEDIUM = 165; // MinScore team-spotted explosive по средней ценной цели.
...
int playerAIWeaponDanger(const BattleItem *item)
{
	...
	auto ruleDanger = [](const RuleItem *rule) -> int
	{
		...
		return std::max(0, rule->getPower()) + accuracy / 2 + radius * 12;
	};
	...
	for (BattleActionType ammoAction : ammoActions)
	{
		...
		{
			danger = std::max(danger, ruleDanger(ammo->getRules()));
		}
	}
	return danger;
}
...
int playerAIWeaponDirectDanger(const BattleItem *item)
{
	...
	return std::max(0, rule->getPower())
...
int playerAIWeaponBlastRadius(const BattleItem *item)
{
	...
	{
		return rule ? std::max(0, rule->getExplosionRadius({ BA_NONE })) : 0;
	};
...
bool playerCanThrowFromTile(SavedBattleGame *save, BattleUnit *unit, BattleItem *item, const Position &from, const Position &target)
{
	...
	double maxDistance = (ProjectileFlyBState::getMaxThrowDistance(weight, unit->getBaseStats()->strength, zlevel) + 8) / 16.0;
	...
	Position targetVoxel = target.toVoxel() + Position(8, 8, (2 + -targetTile->getTerrainLevel()));
	Position targetPos = targetVoxel.toTile();
	double curvature = std::max(0.48, 1.73 / sqrt(sqrt((double)unit->getBaseStats()->strength / (double)weight)) + (unit->isKneeled() ? 0.1 : 0.0));
...
int getPlayerBestExplosiveReserve(BattleUnit *unit, SavedBattleGame *save, bool urgent, int *bestPower = 0, int *bestRadius = 0, const char **bestType = 0)
{
	...
	for (auto *item : *unit->getInventory())
	{
		...
		const int reserve = throwCost.Time + primeCost.Time + 4;
		const int score = power * 2 + radius * 25 - reserve;
...
bool isRecentPlayerProximityMinePlan(SavedBattleGame *save, UnitFaction faction, const Position &contact, const Position *target = 0, int targetSpacing = 7)
{
...
void recordPlayerProximityMinePlan(SavedBattleGame *save, UnitFaction faction, const Position &contact, const Position &target)
{
	...
	for (const auto &plan : pendingPlayerProximityMinePlans)
	{
		if (plan.faction == faction
			...
			&& Position::distance2d(plan.target, target) <= 7)
		{
			return;
		}
	}
...
bool isPendingPlayerGrenadeDanger(SavedBattleGame *save, UnitFaction faction, const Position &pos)
{
	...
	for (const auto &danger : pendingPlayerGrenadeDangers)
	{
		if (danger.faction == faction
			...
			&& Position::distance2d(danger.target, pos) <= danger.radius)
		{
			return true;
		}
	}
...
bool isActivePlayerProximityMineDanger(SavedBattleGame *save, const Position &pos, int margin = 0)
{
	...
	for (int i = 0; i < save->getMapSizeXYZ(); ++i)
	{
		...
		if (!tile || abs(tile->getPosition().z - pos.z) > Options::battleExplosionHeight)
		{
			continue;
		}
		for (auto *item : *tile->getInventory())
		{
			...
			if (item->getFuseTimer() < 0 && !item->isFuseEnabled())
			{
				continue;
			}
			BattleAction mineAction;
			mineAction.weapon = item;
			mineAction.type = BA_THROW;
			const int radius = item->getRules()->getExplosionRadius(BattleActionAttack::GetBeforeShoot(mineAction));
			if (radius > 0 && Position::distance2d(tile->getPosition(), pos) <= radius + margin)
			{
				return true;
			}
		}
...
PlayerFactionAI::PlayerFactionAI(SavedBattleGame *save, BattleUnit *unit, Node *node) :
	...
	_stalkAmbushAction(false), _cleanShotMoveAction(false), _fallbackCoverAction(false), _factionSupportMoveAction(false), _factionAI(0)
{
...
PlayerFactionAI::PlayerAITacticalRole PlayerFactionAI::getPlayerTacticalRole(BattleItem *weapon, PlayerAIRole role) const
{
	...
	const int selfScore = (int)stats->tu * 2 + (int)stats->reactions + _unit->getHealth() * 80 / maxHealth - (weapon ? weapon->getTotalWeight() : 0);
	int betterScouts = 0;
	for (auto *other : *_save->getUnits())
	{
		...
		const int otherScore = (int)otherStats->tu * 2 + (int)otherStats->reactions + other->getHealth() * 80 / otherMaxHealth - (otherWeapon ? otherWeapon->getTotalWeight() : 0);
		if (otherScore > selfScore + 8)
		{
			++betterScouts;
		}
	}
...
void PlayerFactionAI::think(BattleAction *action)
{
	...
	if (_unit->getFaction() == FACTION_PLAYER)
	{
		for (auto *other : *_save->getUnits())
		{
			...
			else if (other->getFaction() == FACTION_HOSTILE)
			{
				...
				playerMaxHostileBlastRadius = std::max(playerMaxHostileBlastRadius, playerAIWeaponBlastRadius(other->getMainHandWeapon(false)));
	...
	if (currentGrenadeDanger && !_escapeTUs)
	{
		setupEscape();
		if (Options::autoBattleLog)
		{
		...
		&& _attackAction.weapon->getRules()->getBattleType() == BT_FIREARM)
	{
		...
		if (durableTarget && poorTrade && exposedOpening && cannotBreakAfterShot && !attackWouldFinish)
		{
			BattleAction savedAttack = _attackAction;
			if (setupFallbackCoverMove(currentExposure >= 90 ? 95 : 115, currentExposure >= 90 ? 20 : 35, _spottingEnemies > 0 ? 1 : 0, 6))
			{
				if (Options::autoBattleLog)
				{
					std::ostringstream log;
	...
	if (_unit->getFaction() == FACTION_PLAYER && _factionAI && _attackAction.type == BA_RETHINK && (_knownEnemies || _factionAI->getEnemyContactCount() > 0))
	{
		...
		int enemiesInRoom = 1;
			...
			if ((playerHeavyExplosiveDoctrine || playerHighHpExplosiveReserve || playerSlowDangerExplosiveReserve)
				...
				&& playerExplosiveThrowerRank < 8)
			{
				const int explosiveReserveMoveTU = std::min(_unit->getBaseStats()->tu - 4, playerExplosiveReserveTU + (playerExplosiveThrowerRank < 3 ? 8 : (playerExplosiveThrowerRank < 6 ? 5 : 2)));
				reserveMoveTU = std::max(reserveMoveTU, explosiveReserveMoveTU);
				if (huntingHiddenContact && !lateHiddenHunt)
				{
					maxSupportMoveDistance = std::min(maxSupportMoveDistance, playerExplosiveThrowerRank < 3 ? 2 : (playerExplosiveThrowerRank < 6 ? 3 : 4));
				}
			}
			...
			for (auto tileIndex : _reachable)
			{
				...
				int supportScore = openAreaFight
					? abs(dist - desiredOpenDist) * 6 + spotters * 40 + moveDist * 3 - candidateCover * 5 + allyCrowdingPenalty(pos) + allyFireLanePenalty(pos)
					: dist * 4 + spotters * 20 + moveDist * 2 - candidateCover * 3 + allyCrowdingPenalty(pos) + allyFireLanePenalty(pos);
...
factionRoomTacticsDone:
	...
	if (_unit->getFaction() == FACTION_PLAYER && !evacuatingGrenadeDanger && _visibleEnemies && _attackAction.type != BA_RETHINK && _attackAction.type != BA_WALK)
	{
			...
			else
			{
				if (Options::autoBattleLog)
				{
					...
					log << "Player faction explosive action rejected before throw: unit=" << _unit->getId()
						...
						<< ", reason=not_enough_tu_after_prime";
					_save->appendToAutoBattleLog(log.str());
				}
				action->type = BA_NONE;
		...
		&& !currentGrenadeDanger && !firedThisTurn)
	{
			...
			&& !_spottingEnemies)
		{
			...
			{
				std::ostringstream log;
				log << "Player faction initial deployment hold: unit=" << _unit->getId()
					<< ", position=" << _unit->getPosition()
					<< ", rejectedTarget=" << action->target
					<< ", turnMoveCount=" << playerTurnMoveCount
					<< ", turnMoveDistance=" << playerTurnMoveDistance
					...
					<< ", maxHostileHealth=" << playerMaxHostileHealth
					<< ", maxHostileWeaponDanger=" << playerMaxHostileWeaponDanger
					<< ", reason=preserve_first_turn_reaction_after_landing_step";
				_save->appendToAutoBattleLog(log.str());
			}
			action->type = BA_NONE;
			action->target = _unit->getPosition();
		...
		else if ((playerHeavyExplosiveDoctrine || playerHighHpExplosiveReserve) && playerExplosiveReserveTU > 0 && playerExplosiveThrowerRank < 6 && playerTurnMoveCount > 0)
		{
		...
		if ((_spottingEnemies || earlyContactPressure || hiddenContactPressure) && repeatedContactMove)
		{
		...
		&& !_visibleEnemies && !_spottingEnemies && _factionAI && _factionAI->getEnemyContactCount() > 0)
	{
		...
		if (_factionAI->getBestEnemyContactPosition(&contactPos, &contactRoom, &enemiesInRoom, &visibleFactionContact))
		{
			...
			const int maxMoveTU = std::max(8, std::min(activeHostiles > 2 ? 18 : 28, _unit->getTimeUnits() - reserveTU));
			const int currentContactDist = Position::distance2d(_unit->getPosition(), contactPos);
			...
			for (auto tileIndex : _reachable)
			{
				...
				if (pos.z != _unit->getPosition().z || tile->getDangerous() || (tile->getUnit() && tile->getUnit() != _unit))
				{
					continue;
				}
				if (isPlayerExplosiveDanger(_save, _unit->getFaction(), pos))
				...
				if (!forcedHunt && !visibleFactionContact && contactDist > currentContactDist && cover < currentCover + 10)
				{
					continue;
				}
				const int spotters = getSpottingUnits(pos);
...
int PlayerFactionAI::countKnownTargets() const
{
	...
	if (_unit->getFaction() == FACTION_PLAYER && _factionAI)
	{
		...
		const int plannedContacts = _factionAI->getEnemyContactCount();
		if (plannedContacts > 0 && plannedContacts <= std::max(activeHostiles, (int)_save->getUnits()->size()))
		{
...
int PlayerFactionAI::estimateDirectShotDamage(BattleAction *action, BattleUnit *target, int accuracy, int shots) const
{
	...
	const int power = std::max(0, attack.damage_item->getRules()->getPower());
...
bool PlayerFactionAI::setupCleanShotMove(BattleUnit *target)
{
	...
	for (auto tileIndex : reachableTiles)
	{
		...
		const int exposure = getEnemyFireExposure(pos);
		const int fireLines = countEnemyFireLines(pos);
		const int retreatTU = std::max(0, _unit->getTimeUnits() - moveTU - (int)snapCost.Time);
		...
		if (!hasLine)
		{
			...
			if (tacticalRole == TACTICAL_FIRE_SUPPORT)
			{
				score -= std::max(0, preferredRange - dist) * 12;
			}
			else if (tacticalRole == TACTICAL_SCOUT)
			{
				score += std::max(0, currentDist - dist) * 6;
				score -= std::max(0, moveDistance - 4) * 8;
			}
			if (hasRetreatRoute && needsRetreatRoute)
			{
				score += 25;
...
bool PlayerFactionAI::setupFallbackCoverMove(int minScore, int minExposureGain, int minSpotterGain, int minCoverGain)
{
	...
	const bool repeatedLowPressureFallback = turnMoveCount >= 2
		...
		&& crowdingGain < 80
		&& laneGain < 80;
	if (bestPos == current || bestScore < minScore
	...
	{
		if (Options::autoBattleLog && repeatedLowPressureFallback)
		{
			std::ostringstream log;
			log << "Player faction fallback loop hold: unit=" << _unit->getId()
				<< ", position=" << current
...
bool PlayerFactionAI::explosiveProjectileRiskyForAllies(BattleAction *action, int radius, const Position *originPosition, bool logRejection, std::string *rejectReason, bool *hardReject) const
{
	...
	auto canEvacuateBlast = [&](BattleUnit *unit, const Position &impactTile) -> bool
	{
		if (!unit || unit == action->actor || unit->isOut() || unit->getFaction() != action->actor->getFaction())
		{
			return false;
		}
		if (abs(unit->getPosition().z - impactTile.z) > Options::battleExplosionHeight)
		{
			return true;
		}
		const int dist = Position::distance2d(unit->getPosition(), impactTile);
		if (dist > radius)
		{
			return true;
		}
		const int requiredSteps = radius - dist + 1;
		return unit->getTimeUnits() >= std::max(8, requiredSteps * 6);
	};
	...
	auto blastWouldHitAlly = [&](const Position &impactTile) -> bool
	{
		for (auto* unit : *_save->getUnits())
		{
			...
			if (abs(unitPosition.z - impactTile.z) <= Options::battleExplosionHeight &&
				...
				!canEvacuateBlast(unit, impactTile))
			{
				return true;
			}
		}
	...
	for (size_t i = 0; i < targetSamples.size(); ++i)
	{
		...
		if (risky)
		{
			if (i == 0)
			{
				bool centralHardReject = firstReason == "ally_on_trajectory" || firstImpactTile == actorPosition;
...
bool PlayerFactionAI::directProjectileRiskyForAllies(BattleAction *action, BattleUnit *target, bool logRejection) const
{
	...
	if (lenSq > PLAYER_AI_GEOMETRY_EPSILON)
	{
		for (auto *ally : *_save->getUnits())
		{
			...
			const double directLaneThreshold = action->type == BA_AIMEDSHOT ? 0.85 : 1.45;
...
int PlayerFactionAI::scorePlayerGrenadeTarget(BattleItem *grenade, const Position &targetPos, int radius, bool proximity, std::string *rejectReason) const
{
	...
	const bool instantBlast = !proximity
		&& (Options::battleInstantGrenade
			|| grenade->getRules()->getFuseTimerType() == BFT_INSTANT
	...
	const int throwerPostThrowTU = proximity ? std::max(0, _unit->getTimeUnits() - action.Time - _unit->getActionTUs(BA_PRIME, grenade).Time - 4) : 0;
	int enemiesAffected = 0;
	...
	auto canEvacuateBlast = [&](BattleUnit *unit) -> bool
	{
		...
		return availableTU >= std::max(8, requiredSteps * 6);
	};
	auto riskyBlastEvacuation = [&](BattleUnit *unit) -> bool
	{
		...
		const int requiredSteps = allySafetyRadius - dist + 1;
		const int requiredTU = std::max(8, requiredSteps * 6);
		return dist <= std::max(1, allySafetyRadius / 2) || unit->getTimeUnits() < requiredTU + 10 || getSpottingUnits(unit->getPosition()) > 0;
	};
	for (auto *bu : *_save->getUnits())
	{
		if (!bu || bu->isOut())
	...
	if (!proximity)
	{
		...
		if (projectileScatterRisk)
		{
			...
			if (score < 260 && enemiesAffected < 3 && highValueEnemiesAffected < 2)
			{
		...
		if (enemiesAffected == 1)
		{
			...
			else
			{
				...
				BattleItem *enemyWeapon = target->getMainHandWeapon(false);
				const int enemyWeaponDanger = playerAIWeaponDirectDanger(enemyWeapon);
				if (target->getHealth() >= 60 || armor >= power / 2)
				{
					score += 150;
				}
				if (target->getHealth() >= 100 || armor >= power || enemyWeaponDanger > 120)
				{
					score += 150;
				}
			}
...
bool PlayerFactionAI::setupProximityMineAmbush()
{
	...
	auto allyUnsafeForMine = [&](BattleUnit *ally, const Position &pos, int radius, int margin = 0, int throwerAvailableTU = -1) -> bool
	{
		...
		const int requiredSteps = std::max(1, radius - dist + 1);
		return ally->getTimeUnits() < std::max(8, requiredSteps * 6);
	};
	...
	if (!_spottingEnemies && getPlayerProximityMineStaging(_save, _unit->getId(), &stagedTarget, &stagedContact))
	{
		...
		const int effectiveThrowLimit = throwLimit + 1;
		...
		{
			blocked = true;
			blockedReason = !targetTile ? "missing_target_tile" : "friendly_on_target";
		}
		for (auto *ally : *_save->getUnits())
		{
			if (allyUnsafeForMine(ally, stagedTarget, stagedRadius, 1, throwerPostThrowTU))
			{
				blocked = true;
	...
	const bool earlyPressureFieldMine = _save->getTurn() <= 3
		&& _knownEnemies >= 8
		&& !_visibleEnemies
		&& !_spottingEnemies
		&& activeHostiles >= std::max(8, activeAllies / 2);
	...
	{
		if (pos.z != _unit->getPosition().z || !_save->getTile(pos) || isPlayerExplosiveDanger(_save, _unit->getFaction(), pos) || tileHasProximityMine(pos))
		{
			return;
		}
	...
	for (auto *mine : *_unit->getInventory())
	{
		...
		const int throwLimit = std::max(6, throwAccuracy / 12);
		const int effectiveThrowLimit = throwLimit + (fieldMine ? 1 : 0);
		...
		for (const auto &target : candidates)
		{
			...
			if (fieldMine)
			{
				...
				{
					tacticalScore += std::min(95, dot / 2);
					tacticalScore -= std::min(130, cross * 3 / unitContactDist);
				}
			}
			...
			const bool lowAccuracyTooFar = throwAccuracy < 65
				&& throwDist > (fieldMine ? std::max(5, effectiveThrowLimit - 1) : 4);
			const bool relaxedFieldMineThrow = fieldMine && _knownEnemies >= 12;
	...
	if (bestAction.type == BA_RETHINK || bestScore < 140)
	{
		if (bestStageMine && bestStageScore >= 145 && !_reachable.empty())
		{
			...
			for (auto tileIndex : _reachable)
			{
				...
				if (futureThrowDist < std::max(2, bestStageRadius - 1))
				{
					continue;
				}
